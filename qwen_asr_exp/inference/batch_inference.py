"""Batch inference for Qwen3-ASR-1.7B with bucketed audio padding.
...
prefill compiles once per bucket instead of once per unique audio length.
...
Usage (via multihost_runner.py from jumpbox):
    python3 qwen_asr_exp/inference/batch_inference.py <config.yml> [key=value ...]

Output: gs://arabic-asr-dataset/distillation/asr_inference_results.jsonl
"""
...
# Qwen3-ASR special token IDs (different from qwen3-omni)
QWEN3_ASR_AUDIO_START_TOKEN = 151669  # <|audio_start|>
QWEN3_ASR_AUDIO_END_TOKEN = 151670    # <|audio_end|>
QWEN3_ASR_AUDIO_TOKEN = 151676        # <|AUDIO|>

# --- Configuration ---
GCSFUSE_BASE = "/tmp/gcsfuse/grain_data_arrayrecord"
OUTPUT_DIR = "/tmp/distillation"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "asr_inference_results.jsonl")
GCS_OUTPUT = "gs://arabic-asr-dataset/distillation/asr_inference_results.jsonl"
NUM_SAMPLES = 5000
SEED = 42
N_WINDOW = 50
...
def build_prompt_tokens(hf_tokenizer, num_audio_tokens):
  """Build tokenized prompt with audio placeholders for ASR transcription."""
  ...
  audio_block = (
      [QWEN3_ASR_AUDIO_START_TOKEN]
      + [QWEN3_ASR_AUDIO_TOKEN] * num_audio_tokens
      + [QWEN3_ASR_AUDIO_END_TOKEN]
  )
